...
let loadingMnist = tf.data
  .csv(mnistUrl, { columnConfigs: { label: { isLabel: true } } })
  .map(({ xs, ys }) => {
...
let loadingCifarX = fetch(cifarXUrl)
  .then((res) => res.json())
  .then((arr) => (cifarX = tf.data.array(arr).batch(1)));
let loadingCifarY = fetch(cifarYUrl)
  .then((res) => res.json())
  .then((arr) => (cifarY = tf.data.array(arr).batch(1)));
let loadingCifar = Promise.all([loadingCifarX, loadingCifarY])
  .then(() => tf.data.zip([cifarX, cifarY]).toArray())
  .then(
    (ds) =>
      (cifarDataset = ds.map((e) => {
...
let loadingGtsrbX = fetch(gtsrbXUrl)
  .then((res) => res.json())
  .then((arr) => (gtsrbX = tf.data.array(arr).batch(1)));
let loadingGtsrbY = fetch(gtsrbYUrl)
  .then((res) => res.json())
  .then((arr) => (gtsrbY = tf.data.array(arr).batch(1)));
let loadingGtsrb = Promise.all([loadingGtsrbX, loadingGtsrbY])
  .then(() => tf.data.zip([gtsrbX, gtsrbY]).toArray())
  .then(
    (ds) =>
      (gtsrbDataset = ds.map((e) => {
        return { xs: e[0], ys: e[1] };
      }))
  );
...
let imagenetY = imagenetYLbls.map((lbl) =>
  tf.oneHot(lbl, 1000).reshape([1, 1000])
);
...
loadedImagenetData.then(() => {
  for (let i = 0; i < imagenetXUrls.length; i++) {
    ...
    imagenetX.push(
      tf.browser.fromPixels(img).div(255.0).reshape([1, 224, 224, 3])
    );
  }
});
...
async function predict() {
  ...
  if (modelName === "mnist") {
    ...
    _predict(
      imagenetModel,
      imagenetX[imagenetIdx],
      imagenetYLbls[imagenetIdx],
      IMAGENET_CLASSES
    );
  }
  ...
  function _predict(model, img, lblIdx, CLASS_NAMES) {
    ...
    // Display prediction
    let status = {
      msg: "✅ Prediction is correct.",
      statusClass: "status-green",
    }; // Predictions on the sample should always be correct
    showPrediction(
      `<p>Prediction: "${CLASS_NAMES[predLblIdx]}"</p>
      <p>Probability: ${(predProb * 100).toFixed(2)}% </p>`,
      status
    );
  }
}
...
async function generateAdv() {
  ...
  if (modelName === "mnist") {
    ...
    await _generateAdv(
      mnistModel,
      mnistDataset[mnistIdx].xs,
      mnistDataset[mnistIdx].ys,
      MNIST_CLASSES,
      MNIST_CONFIGS[attack.name]
    );
  } else if (modelName === "cifar") {
    await loadCifarModel();
    await loadingCifar;
    await _generateAdv(
      cifarModel,
      cifarDataset[cifarIdx].xs,
      cifarDataset[cifarIdx].ys,
      CIFAR_CLASSES,
      CIFAR_CONFIGS[attack.name]
    );
  } else if (modelName === "gtsrb") {
    await loadGtsrbModel();
    await loadingGtsrb;
    await _generateAdv(
      gtsrbModel,
      gtsrbDataset[gtsrbIdx].xs,
      gtsrbDataset[gtsrbIdx].ys,
      GTSRB_CLASSES,
      GTSRB_CONFIGS[attack.name]
    );
  } else if (modelName === "imagenet") {
    await loadImagenetModel();
    await loadedImagenetData;
    await _generateAdv(
      imagenetModel,
      imagenetX[imagenetIdx],
      imagenetY[imagenetIdx],
      IMAGENET_CLASSES,
      IMAGENET_CONFIGS[attack.name]
    );
  }
  ...
  async function _generateAdv(model, img, lbl, CLASS_NAMES, CONFIG) {
    ...
    let predLblIdx = pred.argMax(1).dataSync()[0];
    let predProb = pred.max().dataSync()[0];
    advPrediction = `<p>Prediction: "${CLASS_NAMES[predLblIdx]}"</p>
    <p>Probability: ${(predProb * 100).toFixed(2)}%</p>`;
    ...
    if (predLblIdx === targetLblIdx) {
      advStatus = {
        msg: "❌ Prediction is wrong. Attack succeeded!",
        statusClass: "status-red",
      };
    } else if (predLblIdx !== lblIdx) {
      advStatus = {
        msg: "❌ Prediction is wrong. Attack partially succeeded!",
        statusClass: "status-orange",
      };
    } else {
      advStatus = {
        msg: "✅ Prediction is still correct. Attack failed.",
        statusClass: "status-green",
      };
    }
...
async function resetAttack() {
  ...
  if (
    $("#select-model").value === "gtsrb" ||
    $("#select-model").value === "imagenet"
  ) {
...
function resetAvailableAttacks() {
  ...
  function _resetAvailableAttacks(jsma, originalLbl, TARGETS, CLASS_NAMES) {
    ...
    if (jsma === true) {
      ...
      if (selectAttack.value === "jsma") {
        selectAttack.value = "fgsmTargeted";
      }
    }
    ...
    if (selectTarget.getAttribute("data-model") === modelName) {
      ...
      selectTarget.options.forEach((option) => {
        if (parseInt(option.value) === originalLbl) {
...
function showBanners() {
  if (!supports32BitWebGL()) {
    $("#mobile-banner").style.display = "block";
  } else if (!isDesktopChrome()) {
    $("#chrome-banner").style.display = "block";
  }
}
...
async function drawImg(img, element) {
  ...
  if (img.shape[0] === 784) {
    let resizedImg = tf.image.resizeNearestNeighbor(
      img.reshape([28, 28, 1]),
      [224, 224]
    );
